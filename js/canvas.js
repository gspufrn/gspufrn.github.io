(function() {
    ...
    function calcula_comsoal(){
        ...
        pdf.addImage(imgData, 'JPEG', 0, 10,210,130);
        pdf.text(20, 160,'Eficiência : '+melhor['eficiencia']);
        pdf.text(20, 180,'Takt : '+takt);
        pdf.addPage();
        pdf.autoTable({
    ...
    function lista_disponiveis(el,lista){
        lista_retorno = [];
        for(var i = 0;i<lista.length;i++){
            soma = (time_sum(lista[i])+el['tempo'])
            if(soma<=takt){
                if(i == (lista.length - 1)){
            ...
            }else if(soma == el['tempo'] && soma > takt){
                lista_retorno.push(i);
            }
        }
    ...
    function drawStuff(i) {
    	...
    	context.strokeText(i['letter'], i['x']-16, i['y']+16);
			...
			for(var j = 0;j<i['predecessores'].length;j++){
				...
				canvas_arrow(context,ni['x'],ni['y'],np['x'],np['y'],[]);
			}
		context.stroke();
    }
})();